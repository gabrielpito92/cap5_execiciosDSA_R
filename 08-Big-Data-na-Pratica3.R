...
library(arules) #para regras de associação: algoritmo apriori()
library(arulesViz) #visualização das regras de associação
library(htmlwidgets) #ajuda a ajustar os dados html
library(writexl) #gerar arquivos excel com os resultados
options(warn=-1)
...
# Uma forma inteligente de resolver o problema no dataset (valores ausentes)
...
df1 <- dados[linhas_pares, ] #col vazio pq quero todas as colunas
View(df1)
df2 <- dados[linhas_impares, ] #linhas invalidas / valores ausentes
View(df2)
...
sum(is.na(df1$Item02)) #is NA verifica total ausencia de dados, aqui temos dados como o espaço
View(df1)

# Verifica se temos valores ausentes representados por espaço em branco
which(nchar(trimws(df1$Item01))==0) #trim remove os espaços / which: pergunta se tem alguma coisa ai??? - do tipo char por exemplo....
...
df1_two <- df1[!grepl("^\\s*$", df1$Item02), ] #slicing
...
#error aqui: consegui pelo forum
pacote_split <- split(pacote$Item01, 
                      pacote$Item02,
                      pacote$Item03, 
                      pacote$Item04,
                      pacote$Item05, 
                      pacote$Item06, drop = FALSE) #vai trabalhar apenas com 6 das 20 cols do dataframe

View(pacote_split)

# Transações
?as
transacoes <- as(pacote_split, "transactions") #forçar esses dados como transactions
...
regras_produto1 <- apriori(transacoes, 
                           ...
                           appearance = list(rhs = "Dust-Off Compressed Gas 2 pack", default = "lhs")) #lhs antecedente / rhs consequente
...
regras_produto2 <- apriori(transacoes,
                           ...
                           appearance = list(rhs = "HP 61 ink",default = "lhs")) #lhs antecedente / rhs consequente
...
regras_produto3 <- apriori(transacoes,
                           parameter = list(minlen = 3, conf = 0.5),
                           appearance = list(rhs = "VIVO Dual LCD Monitor Desk mount", default = "lhs")) #lhs antecedente / rhs consequente
...
regras_produto1 <- apriori(transacoes, 
                           parameter = list(minlen = 3, supp = 0.2, conf = 0.5, target = "rules"), #add o param supp
...
df_produto1 <- as(regras_produto1_clean, "data.frame") #forçar a converção para dataframe
View(df_produto1)
